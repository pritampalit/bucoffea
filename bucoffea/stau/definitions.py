...
def stau_accumulator(cfg):
    ...
    tau_pt_ax_wocut = Bin("tau_pt_wocut", r"$p_{T}$ (GeV)", 100, 0, 1000.)
    tau_pt_ax = Bin("tau_pt", r"$p_{T}$ (GeV)", 100, 0, 1000.)

    tau_eta_ax_wocut = Bin("tau_eta_wocut", r"$p_{T}$ (GeV)", 60, -3., 3.)
    tau_deeptauvse_ax_wocut = Bin("tau_deeptauvse_wocut", r"$p_{T}$ (GeV)", 8, 0.5, 8.5)
    tau_deeptauvsmu_ax_wocut = Bin("tau_deeptauvsmu_wocut", r"$p_{T}$ (GeV)", 4, 0.5, 4.5)
    tau_deeptauvsjet_ax_wocut = Bin("tau_deeptauvsjet_wocut", r"$p_{T}$ (GeV)", 8, 0.5, 8.5)

    muon_pt_ax_wocut = Bin("muon_pt_wocut", r"$p_{T}$ (GeV)", 50, 0, 500.)
    muon_eta_ax_wocut = Bin("muon_eta_wocut", r"$p_{T}$ (GeV)", 60, -3., 3.)
    muon_iso_ax_wocut = Bin("muon_iso_wocut", r"$p_{T}$ (GeV)", 25, 0., 1.00)
    muon_absdxy_ax_wocut = Bin("muon_absdxy_wocut", r"$p_{T}$ (GeV)", 25, 0.01, 0.1)
    muon_absdz_ax_wocut = Bin("muon_absdz_wocut", r"$p_{T}$ (GeV)", 20, 0.0, 0.8)

    elec_pt_ax_wocut = Bin("elec_pt_wocut", r"$p_{T}$ (GeV)", 50, 0, 500.)
    elec_eta_ax_wocut = Bin("elec_eta_wocut", r"$p_{T}$ (GeV)", 60, -3., 3.)
    elec_iso_ax_wocut = Bin("elec_iso_wocut", r"$p_{T}$ (GeV)", 25, 0., 1.00)
    elec_absdxy_ax_wocut = Bin("elec_absdxy_wocut", r"$p_{T}$ (GeV)", 25, 0.01, 0.1)
    elec_absdz_ax_wocut = Bin("elec_absdz_wocut", r"$p_{T}$ (GeV)", 20, 0.0, 0.8)

    ak4_pt_ax_wocut = Bin("ak4_pt_wocut", r"$p_{T}$ (GeV)", 100, 0, 1000.)
    ak4_eta_ax_wocut = Bin("ak4_eta_wocut", r"$p_{T}$ (GeV)", 60, -3., 3.)
    bjet_deepjet_ax_wocut = Bin("bjet_deepjet_wocut", r"$p_{T}$ (GeV)", 50, 0., 1.)
    ...
    '''
    multiplicity_ax_wocut = Bin("multiplicity_wocut", r"multiplicity", 10, -0.5, 9.5)
    ...
    mt2_ax = Bin("mt2", r"$M_{T2}$ (GeV)", 50, 0, 1000)
    ...
    '''
    items["tau_pt"] = Hist("Counts", dataset_ax, region_ax, tau_pt_ax)

    items["tau_pt_wocut"] = Hist("Counts", dataset_ax, region_ax, tau_pt_ax_wocut)
    items["tau_eta_wocut"] = Hist("Counts", dataset_ax, region_ax, tau_eta_ax_wocut)
    items["tau_deeptauvse_wocut"] = Hist("Counts", dataset_ax, region_ax, tau_deeptauvse_ax_wocut)
    items["tau_deeptauvsmu_wocut"] = Hist("Counts", dataset_ax, region_ax, tau_deeptauvsmu_ax_wocut)
    items["tau_deeptauvsjet_wocut"] = Hist("Counts", dataset_ax, region_ax, tau_deeptauvsjet_ax_wocut)

    items["muon_pt_wocut"] = Hist("Counts", dataset_ax, region_ax, muon_pt_ax_wocut)
    items["muon_eta_wocut"] = Hist("Counts", dataset_ax, region_ax, muon_eta_ax_wocut)
    items["muon_iso_wocut"] = Hist("Counts", dataset_ax, region_ax, muon_iso_ax_wocut)
    items["muon_absdxy_wocut"] = Hist("Counts", dataset_ax, region_ax, muon_absdxy_ax_wocut)
    items["muon_absdz_wocut"] = Hist("Counts", dataset_ax, region_ax, muon_absdz_ax_wocut)

    items["elec_pt_wocut"] = Hist("Counts", dataset_ax, region_ax, elec_pt_ax_wocut)
    items["elec_eta_wocut"] = Hist("Counts", dataset_ax, region_ax, elec_eta_ax_wocut)
    items["elec_iso_wocut"] = Hist("Counts", dataset_ax, region_ax, elec_iso_ax_wocut)
    items["elec_absdxy_wocut"] = Hist("Counts", dataset_ax, region_ax, elec_absdxy_ax_wocut)
    items["elec_absdz_wocut"] = Hist("Counts", dataset_ax, region_ax, elec_absdz_ax_wocut)

    items["ak4_pt_wocut"] = Hist("Counts", dataset_ax, region_ax, ak4_pt_ax_wocut)
    items["ak4_eta_wocut"] = Hist("Counts", dataset_ax, region_ax, ak4_eta_ax_wocut)
    items["bjet_deepjet_wocut"] = Hist("Counts", dataset_ax, region_ax, bjet_deepjet_ax_wocut)
    ...
    for cand in ['ak4', 'muo', 'ele', 'tau', 'sv']:
        items[f"{cand}_mult"] = Hist(cand, dataset_ax, region_ax, multiplicity_ax)
...
def setup_candidates(df, cfg):
    ...
    muons = JaggedCandidateArray.candidatesfromcounts(
        ...
        dxy=df['Muon_dxy'],
        dz=df['Muon_dz'],
        absdxy=np.abs(df['Muon_dxy']),
        absdz=np.abs(df['Muon_dz'])
    )
    ...
    electrons = JaggedCandidateArray.candidatesfromcounts(
        ...
        looseId=(df[cfg.ELECTRON.BRANCH.ID]>=2),
        mediumId=(df[cfg.ELECTRON.BRANCH.ID]>=3),
        tightId=(df[cfg.ELECTRON.BRANCH.ID]>=4),
        dxy=df['Electron_dxy'],
        dz=df['Electron_dz'],
        absdxy=np.abs(df['Electron_dxy']),
        absdz=np.abs(df['Electron_dz']),
        convsafe = df['Electron_convVeto'],
        mistrkhit = df['Electron_lostHits']
    ...
    taus = JaggedCandidateArray.candidatesfromcounts(
        ...
        #mass=0 * df['Tau_pt'],
        mass=df['Tau_mass'],
        charge=df['Tau_charge'],
        decaymode=df[cfg.TAU.BRANCH.ID],
        #iso=df[cfg.TAU.BRANCH.ISO]
        deeptauvse=df[cfg.TAU.BRANCH.DEEPTAUIDVSE],
        deeptauvsmu=df[cfg.TAU.BRANCH.DEEPTAUIDVSMU],
        deeptauvsjet=df[cfg.TAU.BRANCH.DEEPTAUIDVSJET],
        dxy=df['Tau_dxy'],
        dz=df['Tau_dz'],
        absdxy=np.abs(df['Tau_dxy']),
        absdz=np.abs(df['Tau_dz'])
        #svid1=df[cfg.TAU.BRANCH.SVIDX1],
        #svid2=df[cfg.TAU.BRANCH.SVIDX2]
        ##svid1=df['Tau_svIdx1'],
        ##svid2=df['Tau_svIdx2']
    )
    ...
    if not df['is_data']:
        kwargs = {'genpartflav' : df['Tau_genPartFlav'], 'genpartid' : df['Tau_genPartIdx']}
    ...
    ak4 = JaggedCandidateArray.candidatesfromcounts(
        ...
        mass=df['Jet_mass'],
        ##deepjet=df[cfg.AK4.BRANCH.ID],
        deepjet=df['Jet_btagDeepFlavB'],
        ##svid1=df[cfg.AK4.BRANCH.SVIDX1],
        ##svid2=df[cfg.AK4.BRANCH.SVIDX2]
    )

    if cfg.OVERLAP.AK4.TAU.CLEAN:
        ak4 = ak4[object_overlap(ak4, taus, dr=cfg.OVERLAP.AK4.TAU.DR)]

    
    bjets = ak4[
            (ak4.pt > cfg.BTAG.CUTS.PT) \
            & (ak4.abseta < cfg.BTAG.CUTS.ETA) \
            & (ak4.deepjet >= cfg.BTAG.CUTS.DEEPJET.LOOSE)]
    ...
    met_pt = df[f'{met_branch}_pt']
    met_phi = df[f'{met_branch}_phi']
    pv_good = df['PV_npvsGood']


    # secondaryy vertices for displaced tau
    sv = JaggedCandidateArray.candidatesfromcounts(
        df['nSV'],
        pt=df['SV_pt'],
        eta=df['SV_eta'],
        abseta=np.abs(df['SV_eta']),
        phi=df['SV_phi'],
        #mass=0 * df['SV_pt'],
        mass=df['SV_mass'],
        charge=df['SV_charge'],
        ...
        dlen=df['SV_dlen'],
        dxysig=df['SV_dxySig'],
        dlensig=df['SV_dlenSig'],
        absdxy=np.abs(df['SV_dxy']),
        absdlen=np.abs(df['SV_dlen'])
    )
    #return met_pt, met_phi, ak4, bjets, ak8, muons, electrons, taus, photons
    return met_pt, met_phi, pv_good, ak4, bjets, muons, electrons, taus, sv

def stau_regions(cfg):
    stau_common_cuts = [
        'trig_hlt',
        'good_pvs',
        'veto_elecs',
    ...
    regions = {}
    # Signal regions
    regions['sr_prompt'] = stau_common_cuts
    regions['sr_displaced'] =  displacedtau_cuts
