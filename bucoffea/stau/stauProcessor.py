...
from bucoffea.stau.definitions import (
                                          stau_accumulator,
                                          setup_candidates,
                                          stau_regions,
...
def trigger_selection(selection, df, cfg, arrusemet, arruseditau):
    ...
    if cfg.RUN.SYNC: # Synchronization mode
        ...
        if not cfg.DISPLACED:
            selection.add('trig_hlt', (arrusemet & mask_or(df, cfg.MET_TRIGGER)) | (arruseditau & mask_or(df, cfg.DITAU_TRIGGER.DEEPTAU)))
        elif cfg.DISPLACED:
            #print("hlt sum : ", ((arrusemet & mask_or(df, cfg.MET_TRIGGER)) | (~arrusemet & mask_or(df, cfg.DITAU_TRIGGER.CHARGEDISO))).sum())
            selection.add('trig_hlt', (arrusemet & mask_or(df, cfg.MET_TRIGGER)) | (arruseditau & mask_or(df, cfg.DITAU_TRIGGER.CHARGEDISO)))
            selection.add('trig_ditau_disp', mask_or(df, cfg.DISPLACED_TRIGGER))
...
class stauProcessor(processor.ProcessorABC):
    def __init__(self, cfgfilename, blind = True):
        self._cfgfilename = cfgfilename
        self._year=None
        self._blind=False
        self._configure()
        self._accumulator = stau_accumulator(cfg)
    ...
    def _configure(self, df=None):
        ...
        #cfg.SETTINGS_FILE_FOR_DYNACONF = bucoffea_path("config/stau.yaml")
        cfg.SETTINGS_FILE_FOR_DYNACONF = bucoffea_path(self._cfgfilename)

        # Reload config based on year
        if df:
            dataset = df['dataset']
            ##self._year = extract_year(dataset)
            self._year = extract_year_run3(dataset)
            df["year"] = self._year
    ...
    def process(self, df):
        ...
        if not df['is_data']:
            ...
            genjet = setup_gen_jets(df)
            gentau = setup_gen_taus(df)
        ...
        met_pt, met_phi, pv_good, ak4, bjets, muons, electrons, taus, sv = setup_candidates(df, cfg)
        ##bjets = bjets[bjets.looseId]

        #Taus

        '''
        for i, sub_list in enumerate(taus.pt):
            for j, value in enumerate(sub_list):
                print('a[{}][{}] = {}'.format(i, j, value))
        '''

        #print("taupt type : ", type(taus.pt))
        #print("tau type : ", type(taus))
        ##print("taus.pt : ", taus.pt[22])
        #print("taupt cut : ", cfg.TAU.CUTS.PT)
        ##print("taus.pt cut passed : ", (taus.pt[22] > cfg.TAU.CUTS.PT))
        ##print("taus.pt cut passed size: ", taus[(taus.pt > cfg.TAU.CUTS.PT)].counts.sum())
        ##print("taus.pt cut passed any: ", (taus.pt[22] > cfg.TAU.CUTS.PT).any())
        ##print("taus.pt cut passed any size: ", taus[(taus.pt > cfg.TAU.CUTS.PT).any()].counts.sum())

        ##print("taus.deeptauvsjet : ", taus.deeptauvsjet)
        #print("taudeeptauvsjet cut : ", cfg.TAU.CUTS.DEEPTAUVSJET)
        ##print("taus.deeptauvsjet cut passed : ", (taus.deeptauvsjet >= 8))
        ##print("taus.deeptauvsjet cut passed any: ", (taus.deeptauvsjet >= 8).any())

        ##print("####end of details of an event####")

        #----------------- Good and Veto Object Selection -----------------

        good_taus = taus[(taus.pt > cfg.TAU.CUTS.PT) \
                         & (taus.abseta < cfg.TAU.CUTS.ETA) \
                         & ((taus.decaymode == 0) | (taus.decaymode == 1) | (taus.decaymode == 10) | (taus.decaymode == 11))  \
                         & (taus.deeptauvse >= 3 ) \
                         & (taus.deeptauvsmu >= 4) \
                         & (taus.deeptauvsjet >= 8 )
                       ]

        veto_taus = taus[(taus.pt > cfg.TAU.VETO.PT) \
                                     & (taus.deeptauvsjet >= 4 )
            
        ]
        
        veto_muons = muons[(muons.pt > cfg.MUON.CUTS.PT) \
                           & (muons.abseta < cfg.MUON.CUTS.ETA) \
                           & (muons.looseId) \
                           & (muons.iso < cfg.MUON.CUTS.ISO) \
                           & (muons.absdxy < cfg.MUON.CUTS.D0) \
                           & (muons.absdz < cfg.MUON.CUTS.DZ)
                       ]

        
        veto_elecs = electrons[(electrons.pt > cfg.ELECTRON.CUTS.PT) \
                              & (electrons.abseta < cfg.ELECTRON.CUTS.ETA) \
                              & (electrons.mediumId) \
                              & (electrons.iso < cfg.ELECTRON.CUTS.ISO) \
                              & (electrons.absdxy < cfg.ELECTRON.CUTS.D0) \
                              & (electrons.absdz < cfg.ELECTRON.CUTS.DZ)
                          ]                              

        ak4_tau_pairs = ak4.cross(good_taus)
        dr_ak4_tau = np.hypot(ak4_tau_pairs.i0.eta-ak4_tau_pairs.i1.eta , dphi(ak4_tau_pairs.i0.phi,ak4_tau_pairs.i1.phi))
    
        good_ak4 = ak4[(ak4.pt > cfg.AK4.CUTS.PT) \
                           & (ak4.abseta < cfg.AK4.CUTS.ETA) \
                           & (dr_ak4_tau > 0.4).all()
                   ]
        ...
        arrusemet = np.ones(df.size)==1
        arruseditau = np.ones(df.size)==1
        #print("arrusemet before: ", arrusemet)
        #print("df size : ", df.size)
        #print("met_pt : ", met_pt)
        #print("met_pt gt 200 : ", (met_pt > 200))
        #print("met_pt gt 200 if any: ", (met_pt > 200).any())
        ## plot met_pt and check if it is at all gt 200 or not
        #if (met_pt > 200) : 
            #usemethlt = True
        arrusemet = (met_pt > 200) & arrusemet # Met Trigger
        arruseditau = ((met_pt <= 200) & (taus.pt > 40).all()) & arruseditau   # DiTau Trigger
        #elif (met_pt <= 200 & taus.pt > 40) :
            #usemethlt = usemethlt
        #print("arrusemet after: ", arrusemet)
        ####print("arrusemet after if any: ", (arrusemet == 1).sum())
        selection = trigger_selection(selection, df, cfg, arrusemet, arruseditau)
        #selection.add('mu_pt_trig_safe', muons.pt.max() > 30)

        #------------no. of particles selection---------------

        #print("good_taus.counts : ", (good_taus.counts>1).sum())
        #print("veto_elec counts : ", (veto_elecs.counts < 1).sum())
        selection.add('good_pvs', pv_good > 0 )        
        selection.add('veto_elecs', (veto_elecs.counts < 1))
        selection.add('veto_muons', veto_muons.counts < 1)
        selection.add('veto_bjets', veto_bjets.counts < 1 )
        #selection.add('veto_taus', veto_taus.counts < 1 )
        selection.add('veto_taus', veto_taus.counts < 100 )
        #print("good_taus.counts : ", (good_taus.counts>1).sum())
        selection.add('good_taus', good_taus.counts > 1 )

        #selection.add('veto_elecs', veto_elecs.counts < 100)
        #selection.add('veto_muons', veto_muons.counts < 100)
        #selection.add('veto_bjets', veto_bjets.counts < 100 )
        #selection.add('veto_taus', veto_taus.counts < 100 )
        #selection.add('good_taus', good_taus.counts > -1 )

        ditau = good_taus.distincts()
        ###print("ditau : ", (ditau.counts > 1).sum())
        ditau_charge = ditau.i0.charge + ditau.i1.charge
        ###print("ditau charge : ", ((ditau.i0 + ditau.i1).charge != 0).counts.sum())
        #print("ditau charge count : ", (ditau_charge==0).counts.sum())
        selection.add('ditau_charge', (ditau_charge == 0).counts > 0)


        ditau_dphi = dphi(ditau.i0.phi, ditau.i1.phi)
        if not cfg.DISPLACED :
            selection.add('ditau_dphi', (ditau_dphi >= cfg.SELECTION.SIGNAL.DPHITAU).any() )
        else : 
            selection.add('ditau_dphi', (ditau_dphi >= cfg.SELECTION.SIGNAL.DPHITAU_DISP).any() )
            
        selection.add('met_pt', met_pt > cfg.SELECTION.SIGNAL.MINMET)

        ditau_mt1 = mt(ditau.i0.pt, ditau.i0.phi, met_pt, met_phi)
        ditau_mt2 = mt(ditau.i1.pt, ditau.i1.phi, met_pt, met_phi)

        ditau_summt = ditau_mt1 + ditau_mt2
        
        selection.add('ditau_summt', (ditau_summt > cfg.SELECTION.SIGNAL.SUMMT).any())
            

        # stransverse mass, MT2 selection
        total_mt2 = mt2(met_pt, met_phi, ditau.i0.pt, ditau.i0.phi, ditau.i1.pt, ditau.i1.pt)
        pass_all = np.zeros(df.size) == 0
        pass_none = ~pass_all
        ###if (total_mt2 > cfg.SELECTION.SIGNAL.MT2) : 
            ###selection.add('ditau_totalmt2', pass_all)
        ###else : 
        ###    selection.add('ditau_totalmt2', pass_none)
        ...
        ##if (cfg.DISPLACED) :
            # dxypassed = "don't have the variable in nanoaod"
            ##ip3dpassed = np.sqrt(pow(ditau.i0.dxy , 2) + pow(ditau.i0.dz , 2)) > 100 & np.sqrt(pow(ditau.i1.dxy , 2) + pow(ditau.i1.dz , 2)) > 100
            ##selection.add('ditau_disp_ip3d', (ditau > cfg.TAU.DISPLACED.IP3D).any())
        ...
        ##print("region items : ",regions.items())

        for region, cuts in regions.items():

            if 'prompt' in self._cfgfilename:
                if re.search('prompt', region):
                  region = region
                  cuts = cuts
                else : continue  
            elif 'disp' in self._cfgfilename:
                if re.search('disp', region):
                  region = region
                  cuts = cuts
                else : continue
            #print("regions regions first: ", region)
            #if any(x in region for x in ["sr_prompt","sr_displaced"]):
            output['cutflow_' + region][dataset]['all']+=df.size
            #print("regions regions : ", region)
            for icut, cutname in enumerate(cuts):
                    #print("cut flow: ", selection.all(*cuts[:icut+1]))
                    #print("cut flow sum: ", selection.all(*cuts[:icut+1]).sum())
            ...
            #fill_mult('tau_mult_wocut', good_taus)
            
            #fill_mult('prim_taus', taus)
            fill_mult('tau_mult', taus)


            ##fill_mult('veto_tau_mult',veto_taus)
            #fill_mult('prim_electrons',electrons)
            ##fill_mult('veto_electrons',veto_elecs)
            #fill_mult('prim_muons',muons)
            ##fill_mult('veton_muons',veto_muons)
            #fill_mult('prim_ak4',ak4)
            fill_mult('ak4_mult', ak4)
            ##fill_mult('veto_bjets',veto_bjets)
            ...
            ezfill('tau_pt_wocut', tau_pt_wocut=taus.pt.flatten(), weight = weights)
            ezfill('tau_eta_wocut', tau_eta_wocut=taus.eta.flatten(), weight = weights)
            ezfill('tau_deeptauvse_wocut', tau_deeptauvse_wocut=taus.deeptauvse.flatten(), weight = weights)
            ezfill('tau_deeptauvsmu_wocut', tau_deeptauvsmu_wocut=good_taus.deeptauvsmu.flatten(), weight = weights)
            ezfill('tau_deeptauvsjet_wocut', tau_deeptauvsjet_wocut=good_taus.deeptauvsjet.flatten(), weight = weights)
            
            #ezfill('tau_pt', tau_pt=good_taus[mask].pt.flatten(), weight = weights)

            ezfill('muon_pt_wocut', muon_pt_wocut=muons.pt.flatten(), weight = weights)
            ezfill('muon_eta_wocut', muon_eta_wocut=muons.eta.flatten(), weight = weights)
            ezfill('muon_iso_wocut', muon_iso_wocut=muons.iso.flatten(), weight = weights)
            ezfill('muon_absdxy_wocut', muon_absdxy_wocut=muons.absdxy.flatten(), weight = weights)
            ezfill('muon_absdz_wocut', muon_absdz_wocut=muons.absdz.flatten(), weight = weights)

            ezfill('elec_pt_wocut', elec_pt_wocut=electrons.pt.flatten(), weight = weights)
            ezfill('elec_eta_wocut', elec_eta_wocut=electrons.eta.flatten(), weight = weights)
            ezfill('elec_iso_wocut', elec_iso_wocut=electrons.iso.flatten(), weight = weights)
            ezfill('elec_absdxy_wocut', elec_absdxy_wocut=electrons.absdxy.flatten(), weight = weights)
            ezfill('elec_absdz_wocut', elec_absdz_wocut=electrons.absdz.flatten(), weight = weights)
            
            ezfill('ak4_pt_wocut', ak4_pt_wocut=ak4.pt.flatten(), weight = weights)
            ezfill('ak4_eta_wocut', ak4_eta_wocut=ak4.eta.flatten(), weight = weights)
            ezfill('bjet_deepjet_wocut', bjet_deepjet_wocut=ak4.deepjet.flatten(), weight = weights)
            ...
            '''
            ezfill('tau_eta', taueta=good_taus[mask].eta.flatten(), weight = weights)
            ezfill('tau_phi', tauphi=good_taus[mask].phi.flatten(), weight = weights)
            ezfill('tau_decaymode', taudecaymode=good_taus[mask].decaymode.flatten(), weight = weights)
            ezfill('tau_dxy', taudxy=good_taus[mask].dxy.flatten(), weight = weights)
            ezfill('tau_dz', taudz=good_taus[mask].dz.flatten(), weight = weights)


            ezfill('muon_pt', muonpt=veto_muons[mask].pt.flatten(), weight = weights)
            ezfill('muon_eta', muoneta=veto_muons[mask].eta.flatten(), weight = weights)
            ezfill('muon_phi', muonphi=veto_muons[mask].phi.flatten(), weight = weights)
            ezfill('muon_iso', muoniso=veto_muons[mask].iso.flatten(), weight = weights)
            ezfill('muon_dxy', muondxy=veto_muons[mask].dxy.flatten(), weight = weights)
            ezfill('muon_dz', muondz=veto_muons[mask].dz.flatten(), weight = weights)

 
            ezfill('elec_pt', elecpt=veto_elecs[mask].pt.flatten(), weight = weights)
            ezfill('elec_eta', eleceta=veto_elecs[mask].eta.flatten(), weight = weights)
            ezfill('elec_phi', elecphi=veto_elecs[mask].phi.flatten(), weight = weights)
            ezfill('elec_iso', eleciso=veto_elecs[mask].iso.flatten(), weight = weights)
            ezfill('elec_dxy', elecdxy=veto_elecs[mask].dxy.flatten(), weight = weights)
            ezfill('elec_dz', elecdz=veto_elecs[mask].dz.flatten(), weight = weights)


            ezfill('ak4_pt', ak4pt=good_ak4[mask].pt.flatten(), weight = weights)
            ezfill('ak4_eta', ak4eta=good_ak4[mask].eta.flatten(), weight = weights)
            ezfill('ak4_phi', ak4phi=good_ak4[mask].phi.flatten(), weight = weights)
